    ...
    public class CompaniesOverviewViewModel : BasePagingModel
    {
        public CompaniesOverviewViewModel(DbSet<DataModels.Company> companies, int currentPage, int totalItems, string searchText, bool companyDeleted)
        {
            ...
            Companies = companies.Select(x => new CompanyOverviewViewModel()
            {
                ...
                NauticalCrew = x.Ship2Company.Any(y => (y.From < DateTime.Now) && (DateTime.Now <= y.Until)) ? x.Ship2Company.Where(y => (y.From < DateTime.Now) && (DateTime.Now <= y.Until))
                ...
                HotelStaff = x.Ship2Company.Any(y => (y.From < DateTime.Now) && (DateTime.Now <= y.Until)) ? x.Ship2Company.Where(y => (y.From < DateTime.Now) && (DateTime.Now <= y.Until))
                    .SelectMany(z => z.Ship.Crew).Where(c => c.From < DateTime.Now && c.Until >= DateTime.Now)
                    .Sum(r => r.HotelStaff != null ? r.HotelStaff.Value : 0) : 0,

                Reports = x.Reports.Count(),

                ShipsVisited = x.Ship2Company.Any(y => (y.From < DateTime.Now) && (DateTime.Now <= y.Until)) ? x.Ship2Company.Count(y => (y.From < DateTime.Now) && (DateTime.Now <= y.Until) && y.Ship.Reports.Count > 0) : 0
