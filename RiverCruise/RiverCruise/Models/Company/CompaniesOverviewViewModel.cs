    ...
    public class CompaniesOverviewViewModel : BasePagingModel
    {
        public CompaniesOverviewViewModel(DbSet<DataModels.Company> companies, int currentPage, int totalItems, string searchText)
        {
            ...
            Companies = companies.Select(x => new CompanyOverviewViewModel()
            {
                ...
                NauticalCrew = x.Ship2Company.Where(y => (y.From < DateTime.Now) && (DateTime.Now <= y.Until))
                    .SelectMany(z => z.Ship.Crew).Where(c => c.From < DateTime.Now && c.Until >= DateTime.Now)
                    .Sum(r => r.NauticalCrew != null ? r.NauticalCrew.Value : 0),
                HotelStaff = x.Ship2Company.Where(y => (y.From < DateTime.Now) && (DateTime.Now <= y.Until))
                    .SelectMany(z => z.Ship.Crew).Where(c => c.From < DateTime.Now && c.Until >= DateTime.Now)
                    .Sum(r => r.HotelStaff != null ? r.HotelStaff.Value : 0),
